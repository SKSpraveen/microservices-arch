...
mongoose
  .connect(
    "mongodb+srv://sksv:sksvD@cluster0.zychm.mongodb.net/orderDB?retryWrites=true&w=majority&appName=Cluster0"
  )
...
app.post("/order/create-payment-intent", async (req, res) => {
  try {
...
app.post("/order/save-order", async (req, res) => {
  try {
    const { paymentIntentId, totalAmount, items, userId, deliveryAddress } =
      req.body;
    ...
    if (!closestDriver)
      return res.status(400).json({ error: "No suitable driver found" });
...
app.get("/order/recommendations/:userId", async (req, res) => {
  try {
...
// Track order
app.get("/order/track-order/:id", async (req, res) => {
...
app.get("/order/orders/:orderId", async (req, res) => {
  try {
    const { orderId } = req.params;
...
app.get("/order/orders/user/:userId", async (req, res) => {
  try {
    const { userId } = req.params;
...
app.get("/order/orders/driver/:driverId", async (req, res) => {
  try {
...
app.put("/order/orders/:orderId", async (req, res) => {
  try {
    const { orderId } = req.params;
...
app.delete("/order/orders/:orderId", async (req, res) => {
  try {
    const { orderId } = req.params;
...
app.post("/order/orders/:orderId/cancel", async (req, res) => {
  try {
    const { orderId } = req.params;
...
app.post("/order/orders/:orderId/deliver", async (req, res) => {
  try {
    const { orderId } = req.params;
    const updatedOrder = await Order.findByIdAndUpdate(
      orderId,
